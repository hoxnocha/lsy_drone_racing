...
[MODIFIED: 
 ...
 6. [MODIFIED v6.6 - User Req] _build_global_path uses custom sequence:
    - Path: [0, 1, 2, 2, 3, 4, ...]
    - Gate 0 (idx 0) pre-point rotates COUNTER-CLOCKWISE.
    - Gate 2 (idx 2, Pass 1) post-point is FORCED 45-deg CLOCKWISE.
    - Gate 2 (idx 2, Pass 2) pre-point IS the post-point from Pass 1.
 7. [FIX v6.5] Corrected typo 'pos_f' to 'pos_ref' in compute_control.
]
"""
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        self.d_pre = 0.45  # Store globally (from user file)
        self.d_post = 1.2 # Store globally (from user file)
        
        # [MODIFIED v6.2] 路径规划的安全和旋转参数
        # (这应大于或等于 'r_safe' in create_ocp_solver)
        ...
        # [NEW v6.3] “第三个门”(index 2)的强制旋转角度
        self._gate2_pass1_post_rot_deg = 45.0
    ...
    # [MODIFIED v6.6] 替换 _build_global_path
    def _build_global_path(self, initial_obs: dict):
        """
        [MODIFIED v6.6] Pre-computes all pre/post points with custom logic:
        - Path: [0, 1, 2, 2, 3, 4, ...]
        - Gate 0 (idx 0) pre-point rotates COUNTER-CLOCKWISE.
        - Gate 2 (idx 2, Pass 1) post-point is FORCED 45-deg CLOCKWISE.
        - Gate 2 (idx 2, Pass 2) pre-point IS the post-point from Pass 1.
        """
        print("[AttitudeMPC] Pre-computing global reference path (v6.6 - custom logic)...")
        ...
        # 序列是: 0, 1, 2, 2, 3, 4, ..., N-1
        # (假设 "第三个门" = idx 2)
        gate_sequence = [0, 1, 2, 2] + list(range(3, num_gates))
        print(f"[AttitudeMPC] Using custom gate sequence: {gate_sequence}")

        gate2_pass_count = 0
        gate2_pass1_post_pt = None # 用于存储 G2(Pass 1) 的 post 点
        ...
        for gate_idx in gate_sequence:
            ...
            # [NEW v6.6] 跟踪门的特定逻辑
            is_gate_0 = (gate_idx == 0)
            is_gate_2 = (gate_idx == 2)
            is_gate_2_pass_1 = False
            is_gate_2_pass_2 = False
            
            if is_gate_2:
                gate2_pass_count += 1
                if gate2_pass_count == 1:
                    is_gate_2_pass_1 = True
                elif gate2_pass_count == 2:
                    is_gate_2_pass_2 = True
            ...
            # --- 2. [MODIFIED v6.6] 动态计算 Pre-Point ---
            pre_pt = g_c.copy() # 初始化
            
            if is_gate_2_pass_2:
                # [NEW v6.6] G2(Pass 2): pre点 = G2(Pass 1)的post点
                if gate2_pass1_post_pt is None:
                    print(f"[AttitudeMPC] ERROR: G{gate_idx} (Pass 2) cannot find post_pt from Pass 1. Using last_post_pt.")
                    pre_pt = last_post_pt.copy()
                else:
                    print(f"[AttitudeMPC] G{gate_idx} (Pass 2): Setting pre_pt to G2(Pass 1)'s post_pt.")
                    pre_pt = gate2_pass1_post_pt.copy()
            else:
                # [DEFAULT] G0, G1, G2(Pass 1), G3... 的 Pre-Point 逻辑
                base_v_pre_xy = -self.d_pre * n_xy
                ...
                if rotation_attempts > self._obs_rotate_max_attempts:
                    ...
                    print(f"[AttitudeMPC] WARN: pre_pt for G{gate_idx} max rotation ({dir_str}) attempts reached.")

            # --- 3. [MODIFIED v6.6] 动态计算 Post-Point ---
            post_pt = g_c.copy() # 初始化

            if is_gate_2_pass_1:
                # [NEW v6.6] G2(Pass 1): 强制旋转 post 点 45 度
                print(f"[AttitudeMPC] G{gate_idx} (Pass 1): Forcing {self._gate2_pass1_post_rot_deg} deg post_pt rotation.")
                base_v_post_xy = self.d_post * n_xy
                rot_rad = np.deg2rad(self._gate2_pass1_post_rot_deg)
                v_post_xy = self._rotate_xy_vector_clockwise(base_v_post_xy, rot_rad)
                post_pt[:2] += v_post_xy
                
                # [NEW v6.6] 存储这个特殊的 post 点
                gate2_pass1_post_pt = post_pt.copy()
                
                # 额外安全检查 (可选但推荐)
                if has_obstacles:
                    dists_xy = np.linalg.norm(obstacles_pos[:, :2] - post_pt[:2], axis=1)
                    if np.min(dists_xy) < self._obs_safe_radius:
                        print(f"[AttitudeMPC] WARN: Forced post_pt for G{gate_idx} (Pass 1) is in collision!")
            else:
                # [DEFAULT] G0, G1, G2(Pass 2), G3, G4... 的 Post-Point 逻辑
                base_v_post_xy = self.d_post * n_xy
                total_rotation_rad = 0.0
                rotation_attempts = 0
                ...
                if rotation_attempts > self._obs_rotate_max_attempts:
                    print(f"[AttitudeMPC] WARN: post_pt for G{gate_idx} max rotation (CW) attempts reached.")
        ...
        self._start_idx = 1
        print(f"[AttitudeMPC] Global path with {len(self._global_waypoints)} points built (v6.6 - custom logic).")
