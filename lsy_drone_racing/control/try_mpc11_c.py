...
[MODIFIED: 
 ...
 6. [MODIFIED v7.3 - User Req] _build_global_path logic:
    - Path: [0, 1, 2, 3, ...] (Standard sequence)
    - Gate 0 (idx 0) pre-point rotates COUNTER-CLOCKWISE.
    - [REMOVED] Gate 1 (idx 1) post-point now uses standard CW avoidance.
 ...
 9. [MODIFIED v7.2] Tuned d_pre/d_post lists specifically for the 
    4-gate track in 'level2.toml' (G0->G1 is very short).
]
"""
...
def create_ocp_solver(Tf: float, N: int, parameters: dict, verbose: bool = False):
    ...
    Q = np.diag([
        400.0, 400.0, 2500.0,   # pos xyz (X, Y, Z) 
        8.0, 8.0, 8,       # rpy 
        10.0, 10.0, 10.0,    # vel
        5.0, 5.0, 5.0        # drpy
    ])
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        print(f"[AttitudeMPC] Loading v7.3 path logic (tuned for level2.toml 4-gate track).")
        self.d_pre_list = np.array([
            0.45,  # G0 (idx 0): 正常
            0.60,  # G1 (idx 1): (G0->G1 距离 < 0.8m, 必须缩短)
            0.6,  # G2 (idx 2): 正常
            0.45,  # G3 (idx 3): (G2->G3 距离 < 1.2m, 缩短)
        ])
        self.d_post_list = np.array([
            1.5,  # G0 (idx 0): (G0->G1 距离 < 0.8m, 必须缩短)
            1.20,  # G1 (idx 1): (G1->G2 距离长, 允许-45度旋转)
            0.6,  # G2 (idx 2): (G2->G3 距离 < 1.2m, 缩短, +45度旋转)
            1.20,  # G3 (idx 3): 最后一个门
        ])
        ...
        # [MODIFIED v7.3] 自定义门旋转
        # [REMOVED] "第二个门" (idx 1) 的 -45 度旋转已被移除
    ...
    # [MODIFIED v7.3] 替换 _build_global_path
    def _build_global_path(self, initial_obs: dict):
        """
        [MODIFIED v7.3] Pre-computes all pre/post points with custom logic:
        - Path: [0, 1, 2, 3, 4, ...] (Standard sequence)
        - Gate 0 (idx 0) pre-point rotates COUNTER-CLOCKWISE.
        - Gate 1 (idx 1) post-point uses standard CW avoidance.
        - Gate 2 (idx 2) post-point is FORCED 45-deg CLOCKWISE.
        - Added custom "climb point" between post-G2(idx 2) and pre-G3(idx 3).
        - Uses self.d_pre_list[idx] and self.d_post_list[idx] for lengths.
        """
        print("[AttitudeMPC] Pre-computing global reference path (v7.3 - custom rotations)...")
        ...
        for gate_idx in gate_sequence:
            ...
            # --- 3. [MODIFIED v7.3] 动态计算 Post-Point ---
            ...
            if is_gate_2:
                ...
                # [DEFAULT] G0, G1, G3, G4... 的 Post-Point 逻辑 (标准顺时针避障)
                total_rotation_rad = 0.0
                rotation_attempts = 0
        ...
        self._start_idx = 1
        print(f"[AttitudeMPC] Global path with {len(self._global_waypoints)} points built (v7.3 - level2.toml tuned).")
