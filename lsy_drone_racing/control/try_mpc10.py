...
[MODIFIED: 
 ...
 6. [MODIFIED v6.2 - User Req] _build_global_path now checks pre/post points
    against obstacles. If a collision is detected, it *rotates* the point
    clockwise (40 deg, then +5 deg) until it is safe.
]
"""
...
class AttitudeMPC(Controller):
    ...
    # [MODIFIED v6.2] 替换 _build_global_path
    def _build_global_path(self, initial_obs: dict):
        """
        [MODIFIED v6.2] Pre-computes all pre/post points with dynamic *rotational*
        obstacle avoidance.
        - G0 normal uses (G0 - initial_pos)
        - Gk normal uses (Gk - post_{k-1})
        - pre/post points are rotated clockwise (40 deg, then +5 deg) if they
          fall within self._obs_safe_radius of any obstacle.
        """
        print("[AttitudeMPC] Pre-computing global reference path (v6.2 - rotational check)...")
        ...
        for i in range(num_gates):
            ...
            # --- 2. [MODIFIED v6.2] 动态计算 Pre-Point (带旋转避障) ---
            
            # 2.1. 计算 *基础* 向量 (门中心 -> 原始 pre 点)
            ...
            while rotation_attempts <= self._obs_rotate_max_attempts:
                # 2.2. 计算旋转后的向量
                v_pre_xy = self._rotate_xy_vector_clockwise(base_v_pre_xy, total_rotation_rad)
            ...
            if rotation_attempts > self._obs_rotate_max_attempts:
                print(f"[AttitudeMPC] WARN: pre_pt for G{i} max rotation attempts ({self._obs_rotate_max_attempts}) reached.")


            # --- 3. [MODIFIED v6.2] 动态计算 Post-Point (带旋转避障) ---
            
            # 3.1. 计算 *基础* 向量 (门中心 -> 原始 post 点)
            ...
            if rotation_attempts > self._obs_rotate_max_attempts:
                print(f"[AttitudeMPC] WARN: post_pt for G{i} max rotation attempts ({self._obs_rotate_max_attempts}) reached.")
        ...
        self._start_idx = 1
        print(f"[AttitudeMPC] Global path with {len(self._global_waypoints)} points built (obs-aware rotation).")
