...
class RacingPathPlanner:
    ...
    def build_trajectory(
        ...
        smoothing_iterations: int = 5 # [新增] 可调参数：平滑迭代次数
...
class MPCC(Controller):
    ...
    - 路径平滑 (Moving Average Smoothing) - 支持 Auto-tuning
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        # -------------------------------------------------------------
        # [TUNING] 参数读取逻辑 (兼容 dict 和 ConfigDict)
        # -------------------------------------------------------------
        def get_cfg(key, default):
            # 如果 config 对象里有 tuning 字典（simulate脚本注入的），优先用
            if hasattr(config, "tuning") and config.tuning is not None:
                if key in config.tuning:
                    return config.tuning[key]
        ...
        # 读取 Tuning 参数
        self.q_l = float(get_cfg("q_l", 522.32)) 
        self.q_c = float(get_cfg("q_c", 279.45))
        self.q_l_gate_peak = float(get_cfg("q_l_gate_peak", 520.26))
        self.q_c_gate_peak = float(get_cfg("q_c_gate_peak", 764.30))
        self.q_l_obst_peak = float(get_cfg("q_l_obst_peak", 207.83))
        self.q_c_obst_peak = float(get_cfg("q_c_obst_peak", 110.51))
        self.miu = float(get_cfg("miu", 14.33))
        self.w_v_gate = float(get_cfg("w_v_gate", 2.73))
        self.w_v_obst = float(get_cfg("w_v_obst", 2.46))
        
        # [新增] 平滑迭代次数 (Tuning Param)
        # 默认值设为 2 (基于你之前的代码)
        self.smoothing_iterations = int(get_cfg("smoothing_iterations", 2))
    ...
    def _replan(self, obs: dict):
        ...
        self._cached_obstacles = obs["obstacles_pos"]
        # [修改] 传递 self.smoothing_iterations 给 Planner
        traj, duration = self.planner.build_trajectory(
            ...
            smoothing_iterations=self.smoothing_iterations # <--- 注入点
        )
    ...
    def _stage_cost_expression(self):
        ...
        # 使用 __init__ 中读取的 Tuning Parameters
    ...
    def _build_ocp_and_solver(self, Tf: float, N_horizon: int) -> tuple[AcadosOcpSolver, AcadosOcp]:
        ...
        # 具体的 cost 表达式由 _stage_cost_expression 构建，使用 self.q_l 等变量
