...
def create_ocp_solver(Tf: float, N: int, parameters: dict, verbose: bool = False):
    ...
    Q = np.diag([
        100.0, 100.0, 2500.0,   # pos xyz (X, Y, Z) 
        5.0, 5.0, 5,       # rpy 
        1,  1, 1,      #vel
        5.0, 5.0, 5.0        # drpy
    ])
...
class AttitudeMPC(Controller):
    ...
    def _build_global_path(self, initial_obs: dict):
        ...
        Pre-computes a global path using ONLY gate centers.
        All pre/post point logic is REMOVED.
        All navigation/avoidance is now 100% handled by the MPC solver.
        """
        print("[AttitudeMPC] Pre-computing global reference path (Gate Centers ONLY)...")
        gates_pos = np.asarray(initial_obs["gates_pos"], float)
        initial_pos = np.asarray(initial_obs["pos"], float)
        
        all_waypoints = [initial_pos] # Start with drone pos
        ...
        for gate_idx in gate_sequence:
            ...
            # --- [REMOVED] All logic for pre_pt, post_pt, n_xy ---
            # --- [REMOVED] Custom climb point (relied on pre/post) ---
            
            # --- 存储航点 ---
            all_waypoints.append(g_c) # 只添加门框中心点

        self._global_waypoints = np.vstack(all_waypoints)
        self._start_idx = 1 # Start targeting the first gate (G0)
        print(f"[AttitudeMPC] Global path with {len(self._global_waypoints)} points built (Gate Centers ONLY).")
