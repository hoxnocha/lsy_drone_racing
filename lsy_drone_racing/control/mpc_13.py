...
# lifting point between G2 and G3
"""
...
def create_ocp_solver(Tf: float, N: int, parameters: dict, verbose: bool = False):
    ...
    Q = np.diag([
        100.0, 100.0, 3000.0,   # pos xyz (X, Y, Z) 
        5.0, 5.0, 5,       # rpy 
        1,  1, 1,      #vel
        5.0, 5.0, 5.0        # drpy
    ])
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        self.d_post_list = np.array([
            0.90,  # G0 (idx 0): (G0->G1 距离 < 0.8m, 必须缩短)
            1.10,  # G1 (idx 1): (G1->G2 距离长)
            1.20,  # G2 (idx 2): (G2->G3 距离 < 1.2m, 缩短)
            1.20,  # G3 (idx 3): 最后一个门
        ])
    ...
    def _build_global_path(self, initial_obs: dict):
        ...
        - [USER REQ] Added ANOTHER climb point between post-G1(idx 1) and pre-G2(idx 2).
        """
        print("[AttitudeMPC] Pre-computing global reference path (v7.5 - naive path, TWO climb points)...")
        ...
        for gate_idx in gate_sequence:
            ...
            # --- [NEW] 添加在 G2(idx 1) 和 G3(idx 2) 之间的爬升点 ---
            if gate_idx == 2: # 当我们处理 G3(idx 2) 时...
                print(f"[AttitudeMPC] G{gate_idx}: Adding custom climb point after post1.")
                post1 = last_post_pt # 这是 G2(idx 1) 的 post 点
                pre2 = pre_pt      # 这是 G3(idx 2) 刚计算出的 pre 点
                
                # 3D 向量 (pre2 - post1)
                vec_to_pre2 = pre2 - post1
                norm_vec = np.linalg.norm(vec_to_pre2)
                
                climb_pt = post1.copy()
                if norm_vec > 1e-6:
                    unit_vec = vec_to_pre2 / norm_vec
                    # 沿 3D 向量方向前进 0.1 米
                    climb_pt += 0.4 * unit_vec 
                
                # 在计算出的新位置上“上方0.5米” (您可以调整这个 0.5)
                climb_pt[2] += 0.1 
                
                all_waypoints.append(climb_pt)
                print(f"[AttitudeMPC] ... Climb point (G1->G2) inserted at {climb_pt}")


            # [KEPT] 在 G2(idx 2) 和 G3(idx 3) 之间添加爬升点
            if gate_idx == 3: # 当我们处理 G3(idx 3) 时...
                print(f"[AttitudeMPC] G{gate_idx}: Adding custom climb point after post2.")
                post2 = last_post_pt # 这是 G2(idx 2) 的 post 点
                pre3 = pre_pt      # 这是 G3(idx 3) 刚计算出的 pre 点
                ...
                all_waypoints.append(climb_pt)
                print(f"[AttitudeMPC] ... Climb point (G2->G3) inserted at {climb_pt}")
        ...
        self._start_idx = 1
        print(f"[AttitudeMPC] Global path with {len(self._global_waypoints)} points built (v7.5 - naive, TWO tuned dist).")
    ...
    def debug_plot(self, obs: dict):
        ...
        if obstacles_pos.size > 0:
            ...
            r_safe_solver = 0.4 # (这匹配 create_ocp_solver)
