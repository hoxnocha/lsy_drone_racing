...
class MPCC(Controller):
    ...
    def _build_ocp_and_solver(self, Tf, N_horizon, verbose=False):
        ocp = AcadosOcp(); model = self._export_dynamics_model(); ocp.model = model
        self.nx = model.x.rows(); self.nu = model.u.rows(); ocp.solver_options.N_horizon = N_horizon
        ocp.cost.cost_type = "EXTERNAL"
        self.q_l = 200; self.q_c = 100; self.Q_w = 1 * DM(np.eye(3))
        self.q_l_gate_peak = 640; self.q_c_gate_peak = 800; self.q_l_obst_peak = 100; self.q_c_obst_peak = 200
        self.R_df = DM(np.diag([0.1, 0.5, 0.5, 0.5])); self.miu = 8.0; self.w_v_gate = 4.0; self.w_v_obst = 1.0
        ocp.model.cost_expr_ext_cost = self._stage_cost_expression()
        thrust_min = float(self._dyn_params["thrust_min"]) * 4.0; thrust_max = float(self._dyn_params["thrust_max"]) * 4.0
