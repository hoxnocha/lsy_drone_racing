...
class MPCC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        self._raptor_opt = RaptorOptimizer(n_ctrl_points=40)
    ...
    def _rebuild_nominal_path_gate(self, obs: dict[str, NDArray[np.floating]]):
        print(f"T={self._step_count / self._ctrl_freq:.2f}: (Re)building path using RAPTOR Optimization...")
        ...
        # 2. 几何倒圆角 (Heuristic Smoothing)
        # 这一步保留，因为它为优化器提供了很好的初始拓扑结构
        ...
        # 3. 几何避障 (Geometric Push-out) -> 得到 "Guide Path"
        # 这里的 safe_dist 设为 0.25，给优化器留出一点被拉回的空间
        # collision_free_wps 虽然无碰撞，但是可能有尖角 (Kinks)
        t_axis_guide, collision_free_wps = self._inject_obstacle_detours(
            with_gate_detours, obstacle_positions, safe_dist=0.25
        )

        # 4. RAPTOR 优化 (B-Spline Optimization)
        # 如果点太少，优化没有意义，直接回退
        if len(collision_free_wps) < 4:
            print("[MPCC] Warning: Path too short for optimization, fallback to spline.")
            self.trajectory = self._path_utils.spline_through_points(self._planned_duration, collision_free_wps)
        else:
            try:
                # 调用优化器
                # smooth_weight: 调大这个值轨迹会更圆滑，但可能更偏离 Guide
                # obs_weight: 调大这个值避障排斥力更强
                bspline_traj = self._raptor_opt.optimize(
                    guide_wps=collision_free_wps,
                    obstacles_pos=obstacle_positions,
                    obs_radius=0.25,
                    smooth_weight=10.0, 
                    guide_weight=1.0,
                    obs_weight=50.0
                ...
                # 5. 转换回 CubicSpline (Compatibility Bridge)
                # 为了兼容 self.trajectory 的接口 (PathTools 需要 CubicSpline)
                # 我们对优化后的 B-Spline 进行密集采样，然后重新拟合 CubicSpline
                # 这样既保留了优化的形状，又适配了原有代码结构
                t_dense = np.linspace(0, 1, 200) # 200个点足够保持形状
                pts_dense = bspline_traj(t_dense)
                
                # 注意：这里我们通过 spline_through_points 重新分配时间 t
                # 这有助于修正 B-Spline 参数化不均匀的问题
                self.trajectory = self._path_utils.spline_through_points(self._planned_duration, pts_dense)
                
            except Exception as e:
                print(f"[MPCC] RAPTOR Optimization failed: {e}. Using geometric fallback.")
                # 优化失败时的回退方案：直接使用几何推离的结果
                self.trajectory = CubicSpline(t_axis_guide, collision_free_wps)

        # 更新总时长，供后续 arclength 重参数化使用
    ...
    def compute_control(
        ...
        # 全状态: [X_phys, r_cmd_state, p_cmd_state, y_cmd_state, f_cmd_state, theta]
