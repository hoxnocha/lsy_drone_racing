...
class AttitudeMPC(Controller):
    ...
    def _build_local_ref(
        ...
        """
        Construct a polyline ref: [cur] -> pre(gate) -> gate_center -> post(gate),
        with pre/post along the gate's **normal in XY** if gate quaternion is available;
        otherwise fallback to track direction (prev->gate and gate->next).
        Returns pos_ref (N+1,3), vel_ref (N+1,3), yaw_ref (N+1,).
        """
        T = self._T_HORIZON
        N = self._N
        dt = self._dt
        ...
        # Decide the direction in XY to place pre/post
        ...
        if use_gate_normal:
            ...
            if gates_pos is not None:
                ...
                if gate_idx + 1 is not None and gate_idx + 1 < len(gates_pos):
                    vec_to_next = gates_pos[gate_idx + 1][:2] - gate_c[:2]
                else:
                    vec_to_next = gate_c[:2] - cur_pos[:2]
                if np.linalg.norm(vec_to_next) > 1e-9 and np.dot(n_xy, vec_to_next) < 0:
                    n_xy = -n_xy
        ...
        # avoid d_pre/d_post exceeding neighbor spacing
        ...
        # r_pre_tol = 0.18
        # signed_along = np.dot((cur_pos[:2] - pre_pt[:2]), n_xy)
        # use_pre = True
        # if np.linalg.norm(cur_pos[:2] - pre_pt[:2]) < r_pre_tol:
        #     use_pre = False
        # elif signed_along > 0.0:
        #     use_pre = False

        # if use_pre:
        #     waypoints = np.vstack([
        #         np.asarray(cur_pos, float).reshape(3),
        #         pre_pt,
        #         gate_c,
        #         post_pt,
        #     ])
        ...
        signed_plane = np.dot(np.asarray(cur_pos, float)[:2] - gate_c[:2], n_xy)

        r_gate = 0.30  
        near_gate_center = np.linalg.norm(np.asarray(cur_pos, float)[:2] - gate_c[:2]) < r_gate

        r_pre_tol = 0.18
        signed_along_pre = np.dot((np.asarray(cur_pos,float)[:2] - pre_pt[:2]), n_xy)
        use_pre = True
        if np.linalg.norm(np.asarray(cur_pos,float)[:2] - pre_pt[:2]) < r_pre_tol:
            use_pre = False
        elif signed_along_pre > 0.0:
            use_pre = False

        if signed_plane < 0.0 and not near_gate_center:
            if use_pre:
                waypoints = np.vstack([
                    np.asarray(cur_pos, float).reshape(3),
                    pre_pt,
    ...
    def debug_plot(self, obs: dict):
        ...
        pos_ref, vel_ref, yaw_ref = self._build_local_ref(
            cur_pos=cur_pos, goal_pos=goal_pos, cur_quat=cur_quat,
            gates_pos=gates_pos, gate_idx=gate_idx,
            v_des=1.2, d_pre=0.8, d_post=0.8,
    ...
    def compute_control(
        ...
        # set obstacle params p for all shooting nodes
        '''
        n_obs = 4
