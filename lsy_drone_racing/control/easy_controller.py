...
[MODIFIED: 
 ...
 6. [MODIFIED v7.6 - User Req]
    ...
    - [FIX] Fixed bug in _build_global_path (was calling non-existent helper).
...
class AttitudeMPC(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        # [COMMENT] 我们假设 obs["..._visited"] 的顺序与 "..._pos" 列表一致
        self._gate_visited = np.zeros(num_gates, dtype=bool)
        self._obstacle_visited = np.zeros(num_obstacles, dtype=bool)
        
        # [COMMENT] _known_..._pos 存储我们对世界的 *最佳* 认知
        # 它首先被初始化为 obs 中的 *标称* (nominal) 位置
        ...
        # [COMMENT] 当传感器看到新物体时，此标志将触发重规划
        self._replan_requested = False 

        # --- [MODIFIED v7.2] 针对 'level2.toml' 中的 4 个门进行调优 ---
        print(f"[AttitudeMPC] Loading v7.6 path logic (tuned for level2.toml 4-gate track).")
        self.d_pre_list = np.array([
            0.7,  # G0 (idx 0): 正常
            0.70,  # G1 (idx 1): (G0->G1 距离 < 0.8m, 必须缩短)
            0.7,  # G2 (idx 2): 正常
            0.7,  # G3 (idx 3): (G2->G3 距离 < 1.2m, 缩短)
        ])
        self.d_post_list = np.array([
            1.20,  # G0 (idx 0): (G0->G1 距离 < 0.8m, 必须缩短)
    ...
    def _build_global_path(self):
        ...
        """
        print("[AttitudeMPC] Re-building global reference path (v7.6)...")
        ...
        for gate_idx in gate_sequence:
            ...
            # [FIXED v7.6] 使用 self._gate_xy_dir 替换缺失的函数
        ...
        self._start_idx = 1
        print(f"[AttitudeMPC] Global path with {len(self._global_waypoints)} points built (v7.6).")
    ...
    def _set_acados_parameters(self, cur_pos, obs, pos_ref, vel_ref, yaw_ref):
        """
    ...
    def compute_control(
        ...
        self._set_acados_parameters(cur_pos, obs, pos_ref, vel_ref, yaw_ref)
