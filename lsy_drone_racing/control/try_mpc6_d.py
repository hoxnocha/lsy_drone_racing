...
class AttitudeMPC(Controller):
    ...
    def _build_local_ref(
        ...
        # --- 3. 构建“向前看”的黄线 (waypoints) ---
        base_waypoints = [cur_pos, pre_pt, gate_c]
        if not is_last_gate:
            base_waypoints.append(post_pt)

            # [新逻辑] 如果有下一个门, 计算它的 pre_pt 并添加
            if next_gate_pos is not None:
                ...
                if next_gates_quat is not None:
                    ...
                    vec_to_next = next_gate_c[:2] - gate_c[:2]
                    if np.linalg.norm(vec_to_next) > 1e-9 and np.dot(next_n_xy, vec_to_next) < 0:
                        next_n_xy = -next_n_xy
                else:
                    vec_in = next_gate_c - gate_c
                    if np.linalg.norm(vec_in) < 1e-6: vec_in = np.array([1.0, 0.0, 0.0])
                    next_n_xy = (vec_in / (np.linalg.norm(vec_in) + 1e-12))[:2]
                
                next_pre_pt = next_gate_c.copy()
                next_pre_pt[:2] -= d_pre * next_n_xy # (使用相同的 d_pre)
                
                base_waypoints.append(next_pre_pt)
            # --- [新逻辑结束] ---
    ...
    def compute_control(
        ...
        # local ref (pre->gate->post)
        # (调用 _build_local_ref (v4.0) 的逻辑保持不变)
        pos_ref, vel_ref, yaw_ref = self._build_local_ref(
            cur_pos=cur_pos,
            goal_pos=goal_pos,
            cur_quat=cur_quat,
